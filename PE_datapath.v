module PE_datapath #(
    ...
    input [packet_width-1:0] input_spike_fifo_din,
    ...
    wire [packet_width-1:0] input_spike_fifo_dout;
    wire [32-1:0] global_to_local_address_mem_output;
    wire [weight_mem_data_width-1:0] local_weight_mem_input, local_weight_mem_output;
    ...
    wire [weight_mem_data_width-1:0] membrane_adder_out, neuron_0_memberane_reg_out, neuron_1_memberane_reg_out, 
        neuron_2_memberane_reg_out, neuron_3_memberane_reg_out, membrane_mux_out;
    wire [weight_mem_data_width-1:0] thr_reg_0_out, thr_reg_1_out, thr_reg_2_out, thr_reg_3_out;
    ...
    wire [weight_mem_data_width - 1:0] weight_shifter_out;
    wire [weight_mem_data_width - 1:0] weight_inverter_out;
    wire [weight_mem_data_width - 1:0] weight_mux_out;
    ...
    // Memory #(.ADDR_WIDTH(global_to_local_address_mem_address_width), .DATA_WIDTH(32), .INIT_FILE(global_to_local_address_mem_init_file)) global_to_local_address_mem(.clk(clk), .rst(rst), .wr_en(global_to_local_address_mem_wr_en), .addr(input_spike_fifo_dout), .din(), .dout(global_to_local_address_mem_output));
    // Memory #(.ADDR_WIDTH(local_32), .DATA_WIDTH(local_weight_mem_data_width), .INIT_FILE(local_weight_mem_init_file)) local_weight_mem(.clk(clk), .rst(rst), .wr_en(local_weight_mem_wr_en), .addr(global_to_local_address_mem_output), .din(local_weight_mem_input), .dout(local_weight_mem_output));
    Memory #(
        .ADDR_WIDTH(10), 
    ...
    shifter #(.number_right_shift(number_right_shift), .DATA_WIDTH(weight_mem_data_width)) weight_shifter(.din(membrane_mux_out), .dout(weight_shifter_out));
    not_each_bit #(.DATA_WIDTH(weight_mem_data_width)) weight_inverter(.in(weight_shifter_out), .out(weight_inverter_out));

    mux2to1 #(.DATA_WIDTH(weight_mem_data_width)) weight_mux(.in0(weight_mem_mux_out), .in1(weight_inverter_out), .sel(wight_mux_sel), .out(weight_mux_out)); // ???? selectesh chie????
    



    Register #(.DATA_WIDTH(weight_mem_data_width)) neuron_0_memberane_reg (.clk(clk), .rst(rst || neuron_0_memberane_reg_wr_rst), .wr_en(neuron_0_memberane_reg_wr_en), .din(membrane_adder_out), .dout(neuron_0_memberane_reg_out));
    Register #(.DATA_WIDTH(weight_mem_data_width)) neuron_1_memberane_reg (.clk(clk), .rst(rst || neuron_1_memberane_reg_wr_rst), .wr_en(neuron_1_memberane_reg_wr_en), .din(membrane_adder_out), .dout(neuron_1_memberane_reg_out));
    Register #(.DATA_WIDTH(weight_mem_data_width)) neuron_2_memberane_reg (.clk(clk), .rst(rst || neuron_2_memberane_reg_wr_rst), .wr_en(neuron_2_memberane_reg_wr_en), .din(membrane_adder_out), .dout(neuron_2_memberane_reg_out));
    Register #(.DATA_WIDTH(weight_mem_data_width)) neuron_3_memberane_reg (.clk(clk), .rst(rst || neuron_3_memberane_reg_wr_rst), .wr_en(neuron_3_memberane_reg_wr_en), .din(membrane_adder_out), .dout(neuron_3_memberane_reg_out));
    ...
    mux4to1 #(
        ...
        .in0(neuron_0_memberane_reg_out), 
        .in1(neuron_1_memberane_reg_out), 
        .in2(neuron_2_memberane_reg_out), 
        .in3(neuron_3_memberane_reg_out), 
        .sel(membrane_mux_sel), 
        .out(membrane_mux_out)
    );
    ...
    Register_initable #(.DATA_WIDTH(weight_mem_data_width), .THRESHOLD_VALUE(thr_reg_init_value)) neuron_0_thr_reg (.clk(clk), .rst(rst), .wr_en(), .din(), .dout(thr_reg_0_out));
    Register_initable #(.DATA_WIDTH(weight_mem_data_width), .THRESHOLD_VALUE(thr_reg_init_value)) neuron_1_thr_reg (.clk(clk), .rst(rst), .wr_en(), .din(), .dout(thr_reg_1_out));
    Register_initable #(.DATA_WIDTH(weight_mem_data_width), .THRESHOLD_VALUE(thr_reg_init_value)) neuron_2_thr_reg (.clk(clk), .rst(rst), .wr_en(), .din(), .dout(thr_reg_2_out));
    Register_initable #(.DATA_WIDTH(weight_mem_data_width), .THRESHOLD_VALUE(thr_reg_init_value)) neuron_3_thr_reg (.clk(clk), .rst(rst), .wr_en(), .din(), .dout(thr_reg_3_out));



    comparator #(.DATA_WIDTH(weight_mem_data_width)) comparator_0 (.a(neuron_0_memberane_reg_out), .b(thr_reg_0_out), .gt_eq(comparator_0_out));
    comparator #(.DATA_WIDTH(weight_mem_data_width)) comparator_1 (.a(neuron_1_memberane_reg_out), .b(thr_reg_1_out), .gt_eq(comparator_1_out));
    comparator #(.DATA_WIDTH(weight_mem_data_width)) comparator_2 (.a(neuron_2_memberane_reg_out), .b(thr_reg_2_out), .gt_eq(comparator_2_out));
    comparator #(.DATA_WIDTH(weight_mem_data_width)) comparator_3 (.a(neuron_3_memberane_reg_out), .b(thr_reg_3_out), .gt_eq(comparator_3_out));
